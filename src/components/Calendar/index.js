  ...
  monthDay(date) {
    ...
    const getDay = function(day) {
      ...
      if (day <= lastMonthDays) {
        ...
        // 本月的
        let dayString =
          day - lastMonthDays < 10 ? '0' + (day - lastMonthDays) : day - lastMonthDays;
        ...
        // 下个月
        let dayString =
          day - (lastMonthDays + currentMonthDays) < 10
            ? '0' + (day - (lastMonthDays + currentMonthDays))
            : day - lastMonthDays;
        let monthString =
          currentMonth + 1 > 12
            ? '01'
        ...
        <Col className="Calendar">
          <Col className="Calendar-title-box">
            <div>
              <Icon
                type="left"
                className={
                  'Calendar-title-ctrl ' + (this.props.param.prevCtrl === false ? 'hide' : '')
                }
            ...
            <div>
              <Icon
                type="right"
                className={
                  'Calendar-title-ctrl ' + (this.props.param.nextCtrl === false ? 'hide' : '')
                }
          ...
          <Col className="Calendar-week-box">
            {this.weekString.map(item => (
              <Col className="Calendar-week-item" key={item}>
                {item}
              </Col>
            ))}
          </Col>
                    ...
                    <Col
                      className={
                        ...
                        (this.props.param.selectDateString === cell.date
                          ? 'Calendar-day-col-item-active '
                          : '') +
                        (cell.month == this.state.month ? 'cur_month ' : '') +
                        (cell.group.length > 0 ? 'cursor' : '')
                      }
                      key={cell.date}
                    ...
                    >
                      <span
                        className={
                          ...
                          (this.props.param.selectDateString === cell.date &&
                          cell.month === this.state.month
                            ? 'Calendar-day-col-day-active'
                            : '')
                        }
                      >
                        {cell.day}
                      ...
                      {cell.group && cell.group.length <= 1 && (
                        <div
                          style={{ height: '28px' }}
                          className={cell.month !== this.state.month ? 'opcity0' : ''}
                        >
                          <span
                            className={
                              'Calendar-day-col-Surplus ' +
                              (this.props.param.selectDateString === cell.date
                                ? 'Calendar-day-col-Surplus-active'
                                : '') +
                              (this.state.month === cell.month ? '' : 'hide')
                            }
                          >
                            {cell.group[0] ? '余: ' + cell.group[0].seat_surplus : ''}
                          </span>
                          <span
                            className={
                              'Calendar-day-col-price ' +
                              (this.props.param.selectDateString === cell.date
                                ? 'Calendar-day-col-price-active'
                                : '') +
                              (this.state.month === cell.month ? '' : 'hide')
                            }
                          >
                            {cell.group[0] ? '￥' + cell.group[0].price : ''}
                          </span>
                        </div>
                      )}
                      {/* 一天多团 */}
                          ...
                          {this.props.param.selectDateString === cell.date &&
                            cell.group.map(group => (
                              <div
                                style={{ height: '28px' }}
                                key={group.id}
                                className={group.id === this.props.param.selectId ? '' : 'hide'}
                              >
                                <span
                                  className={
                                    'Calendar-day-col-Surplus Calendar-day-col-Surplus-active'
                                  }
                                >
                                  {group ? '余: ' + group.seat_surplus : ''}
                                </span>
                                <span
                                  className={'Calendar-day-col-price Calendar-day-col-price-active'}
                                >
                                  {group ? '￥' + group.price : ''}
                                </span>
                              </div>
                            ))}
                          {this.props.param.selectDateString !== cell.date && <div>多团</div>}
                        </div>
                      )}
                    </Col>
                  ))}
            ...
            <div>
              {this.state.much_groups.group &&
                this.state.much_groups.group.map(item => (
                  <div
                    className={
                      ...
                      (this.state._user_select_id
                        ? this.state._user_select_id === item.id
                          ? 'Calendar-modal-list-active'
                          : ''
                        : this.props.param.selectId === item.id
                        ? 'Calendar-modal-list-active'
                        : '')
                    }
                    key={item.id}
                  ...
                  >
                    <span className="Calendar-modal-list-item">
                      日期: {this.state.much_groups.date}
                    </span>
