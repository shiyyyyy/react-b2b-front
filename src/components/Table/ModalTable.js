...
import { renderHeaderBtns, actionColWidth, lastWidth } from './TableUtil';
import { getModalTableConfig, ActionCellModal } from './ModalTableUtil';
...
class ModalTable extends Component {
  ...
  getblockConfig = () => {
    ...
    const config = getModalTableConfig(
      blockConfig,
      rowSelection,
      block,
      dataSource,
    ...
    if (blockConfig.action) {
      ...
      const col = {
        ...
        render: (text, data, index) => (
          <ActionCellModal
            data={data}
            width={actionColWidth(blockConfig, dataSource)}
    ...
    // 添加 table的scroll属性
    // if (!config.scroll) {
    //   // 把第一个clo的width去掉,再添加到总体里,因为都有width可能导致width失效
