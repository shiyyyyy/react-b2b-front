...
const { ScheduleGroupTour,ScheduleBooking,ScheduleTraffic,ScheduleVisa } = ProType;
...
class List extends PureComponent {
  ...
  renderListItem(item){
    ...
    if(item.pd_type == AppConst.PRODUCT_PACKAGETOUR){
        return <ScheduleGroupTour data={item} key={item.id} btnChildren={this.btnChildren(item)} />
    } 
    // eslint-disable-next-line eqeqeq
    if(item.pd_type == AppConst.PRODUCT_ROOM_BOOKING){
        return <ScheduleBooking data={item} key={item.id} btnChildren={this.btnChildren(item)} />;
    }
    // eslint-disable-next-line eqeqeq
    if(item.pd_type == AppConst.PRODUCT_TRAFFIC){
        return <ScheduleTraffic data={item} key={item.id} btnChildren={this.btnChildren(item)} />;
    }
    // eslint-disable-next-line eqeqeq
    if(item.pd_type == AppConst.PRODUCT_VISA_BOOKING){
        return <ScheduleVisa data={item} key={item.id} btnChildren={this.btnChildren(item)} /> ;
    }
    return null;
  }
