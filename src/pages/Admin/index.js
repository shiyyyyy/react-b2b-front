...
@connect(({ adminNotices, adminAnnouncements, loading }) => ({
  adminNotices,
  adminAnnouncements,
...
class AdminIndex extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'adminNotices/fetch',
    });
    dispatch({
      type: 'adminAnnouncements/fetch',
    });
  }
    ...
    const cfg = {
      title: (
        <span>
          销售机会
          <span
            style={{ fontSize: '12px', fontWeight: '400', color: '#B1BCD1', marginLeft: '32px' }}
          >
            (这些搜索记录都不是你想要的?
            ...
            <Col className={styles.ThreeEqualBlock}>
              ...
              <Col xl={6} lg={6} md={10} sm={24} xs={24} className={styles.block}>
                <Row>
                  ...
                  <Col className={styles.content} style={{padding: '12px'}}>
                    <Col span={12} className={styles.c2item}>
                      <Col sm={{ span: 3, push: 6}} md={{span:6, push: 0}} lg={6}><img src={backDatePng} className={styles.icon} alt="日历图标" /></Col>
                      <Col sm={{ span: 9, push: 6}} md={{span: 18, push: 0}} lg={18}>
                        <Col>今日出团</Col>
                        <Col>24</Col>
                      </Col>
                    </Col>
