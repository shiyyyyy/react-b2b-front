  ...
  render() {
    ...
    return (
      <Row>
        <Col>
          {/* 子route容器 */}
          <Col className={route.path === location.pathname ? 'hide' : ''}>
            <Route path="/transaction/change/costHistory" component={Cost} />
          </Col>
          {/* 当前页面 本页面path是route.path, 当前页面path为location.pathname */}
          <Col className={route.path === location.pathname ? '' : 'hide'}>
